...
<li <?php comment_class(empty( $args['has_children'] ) ? '' : 'parent'); ?> id="comment-<?php comment_ID(); ?>">
	...
	<span class="pingtype"><?php comment_type(); ?></span> on <a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>" title="<?php if (function_exists('time_since')) { $comment_datetime = strtotime($comment->comment_date); echo time_since($comment_datetime); ?> ago<?php } else { ?>Permalink to Comment<?php } ?>"><?php printf( __( '%1$s at %2$s', 'twenty-eight' ), get_comment_date( 'M jS, Y' ), get_comment_time() ); ?></a> <?php edit_comment_link('<img src="'.get_bloginfo(template_directory).'/images/pencil.png" alt="Edit Link" />','<span class="commentseditlink">','</span>'); ?>
	</small>
	<?php comment_text(); ?>
<?php
}
