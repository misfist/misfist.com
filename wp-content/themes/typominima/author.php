<?php get_header(); ?>
		...
		<div id="content">
			<div class="archive-title">
				...
				<h5>Entries by <?php echo get_the_author_meta('display_name',get_query_var('author')); ?></h5>
			</div>
		<?php if (have_posts()) : ?>
			...
			<div <?php post_class(); ?> id="post-<?php the_ID(); ?>">
				...
				<div class="post-metas">
					<dl>
						...
						<?php if(isset($typominima['archive_date']) && $typominima['archive_date']==1) { ?>
						<dt><span>Publish date:</span></dt>
						<dd><?php the_time('F jS, Y') ?></dd>						
						<?php } ?>

						<?php if(isset($typominima['archive_comments']) && $typominima['archive_comments']==1) { ?>
						...
						<dd>
							...
							?>
							<a href="<?php the_permalink(); echo $comment_link; ?>"><?php echo $comment_count.' '.$comment_text; ?></a>
						</dd>
						<?php } ?>

						<?php if(isset($typominima['archive_categories']) && $typominima['archive_categories']==1) { ?>
						<dt><span>Categories:</span></dt>
						<dd><?php the_category('<br />'); ?></dd>
						<?php } ?>

						<?php if(isset($typominima['archive_tags']) && $typominima['archive_tags']==1) { ?>
						<dt><span>Tags:</span></dt>
						<dd><?php the_tags('', '<br />', ''); ?> </dd>
						<?php } ?>
					</dl>

					<?php if(isset($typominima['archive_author_info']) && $typominima['archive_author_info']==1) { ?>
					<dl class="mt40">
