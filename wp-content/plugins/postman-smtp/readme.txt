...
Tags: smtp, email, mail, mailer, email log, outgoing mail, oauth2, gmail, google apps, hotmail, yahoo
...
Postman is a next-generation [SMTP Mailer](https://wordpress.org/plugins/search.php?q=smtp), software that assists in the delivery of email generated by your WordPress site. Postman is the first and only plugin to support both traditional passwords and OAuth 2.0, the [modern security mechanism for authentication](http://googleonlinesecurity.blogspot.ca/2014/04/new-security-measures-will-affect-older.html). With OAuth 2.0, there is **no need** to [store your email passsword](http://blog.codinghorror.com/youre-probably-storing-passwords-incorrectly/) in the WordPress database where it might be found.

> v1.7 **Early Beta** adds support for the Mandrill and SendGrid APIs. Send mail to these services even if your host has blocked the standard SMTP ports. [Get it now](https://downloads.wordpress.org/plugin/postman-smtp.zip).

The **Connectivity Test** and intelligent **Setup Wizard** scan your SMTP server to detect firewall blocks and eliminate configuration mistakes. The built-in **Email Log** is an invaluable resource for diagnosing problems with emails that aren't working. Even hosts that block the standard SMTP ports, like GoDaddy or Bluehost, can't stop your Gmail as **Postman will deliver via HTTPS** if it can't use SMTP.
...
* Log all email including message content and server errors
...
= OAuth 2.0 Features =
* Gmail: By combining OAuth2 and the Gmail API over the HTTPS port, Postman can succeed where other plugins fail
* Fire-and-forget delivery continues even if the password changes
* Supports the proprietary implementations of Gmail, Hotmail and Yahoo

> Postman needs [translators](https://translate.wordpress.org/projects/wp-plugins/postman-smtp/dev)! If you are a non-English speaker, please get involved!
...
* Dutch - Louise
* Greek - [Michael Kotsarinis](https://plus.google.com/+MichaelKotsarinis/about)
...
* Connectivity Test against localhost host shows "localhost" instead of "null" if a man-in-the-middle attack is detected
